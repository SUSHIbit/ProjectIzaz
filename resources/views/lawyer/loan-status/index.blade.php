<x-app-layout>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($users as $user)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($user->loanStatus)
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                    {{ $user->loanStatus->status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    {{ ucfirst($user->loanStatus->status) }}
                                                </span>
                                            @else
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-900">{{ $user->loanStatus?->remarks ?? 'No remarks' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $user->loanStatus?->updated_at?->diffForHumans() ?? 'Never' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <button onclick="openUpdateModal({{ $user->id }})" class="text-indigo-600 hover:text-indigo-900">
                                                Update Status
                                            </button>
                                        </td>
                                    </tr>
    ...
    <script>
        function openUpdateModal(userId) {
            ...
            form.action = `/lawyer/loan-status/${userId}/update`;
